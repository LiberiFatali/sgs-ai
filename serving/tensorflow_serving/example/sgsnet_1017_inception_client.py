...
import datetime, json, requests, ssl, StringIO, time, collections
import pytesseract
...
#RACY = ADULT + ['butt-plug', 'dildo', 'pants', 'swimwear', 
	#'underwear', 'bikini, two-piece', 'maillot', 'maillot, tank suit',
	#'brassiere, bra, bandeau', 'miniskirt, mini', 
	#'swimming trunks, bathing trunks']
RACY = ADULT + ['pants', 'swimwear', 
	'bikini, two-piece', 'maillot', 'maillot, tank suit',
	'brassiere, bra, bandeau', 'miniskirt, mini', 
	'swimming trunks, bathing trunks']
...
# for project Moco
MOCO_REJECT = ['penis', 'sex-intercourse', 'vagina', 'womanbreast']
MOCO_REVIEW = ['butt-plug', 'dildo', 'underwear', 'screen, CRT screen', 
	       'studio couch, day bed', 'web site, website, internet site, site']
MOCO_Rx2 = MOCO_REJECT + MOCO_REVIEW

USEFUL_SCORES = [ADULT, ALCOHOL, RACY, VIOLENCE, WEAPON, 
		 MOCO_REJECT, MOCO_REVIEW, MOCO_Rx2]
...
def classify_image():
  ...
  if request.method == 'GET':
    #input_url = request.args.get('url')
    input_url = request.query_string[4:]
    #
    try:
	...
	taskText = imtotext.delay(resp_data)	
	...
	# get classes and probs
	sLabel = result.classes[0:TOP_K]
	fConf = result.scores[0:TOP_K]	
	probs = collections.OrderedDict()
	for c,s in zip(result.classes, result.scores):
	  probs[c] = s
	# prepare result
	sRecognizedText = taskText.get()
	output_info = collections.OrderedDict([('File',input_url), ('Label',sLabel), 
					  ('Probability',fConf), ('Recognized_Text',sRecognizedText)])
	output_score = collections.OrderedDict([ ('Adult',0), 
					  ('Alcohol',0), ('Racy',0), 
					  ('Violence',0), ('Weapon',0), 
					  ('Moco_Reject',0), ('Moco_Review',0), 
					  ('Moco_Rx2',0) ])
	for o, us in zip(output_score, USEFUL_SCORES):
	  output_score[o] = sum([probs[c] for c in us])	
	#
	output = collections.OrderedDict(list(output_info.items()) + list(output_score.items()))
  ...
  ## handle POST
	...
	#input_image_pil = input_image_pil.convert("RGB")
      ...
      #output_obj.close()
    ### extract text asynchronously
    ##sRecognizedText = imtotext.delay(resp_data)
    ## classify with tfserving
    #channel = implementations.insecure_channel(HOST, PORT)
    #stub = inception_inference_pb2.beta_create_InceptionService_stub(channel)
    #grpc_request = inception_inference_pb2.InceptionRequest() 
    #grpc_request.jpeg_encoded = resp_data
    #result = stub.Classify(grpc_request , 60.0)  # 60 secs timeout
    #sLabel = [result.classes[i] for i in xrange(len(result.classes))]
  ...
  #response = Response(response=json.dumps(output, sort_keys=True),
  response = Response(response=json.dumps(output), 
		      mimetype='application/json')
  response.headers.add('Access-Control-Allow-Origin', '*')   
  run_time = time.time() - start_time    
  #logger.info('{0} {1} {2}'.format(datetime.datetime.now(), output, '%.3f'%run_time))
...
###
# text celery helper function
@celery.task(name="celery_tasks.imtotext")
def imtotext(raw_data):
  input_image_pil = Image.open(StringIO.StringIO(raw_data))
  return pytesseract.image_to_string(input_image_pil, lang='some+eng')   
