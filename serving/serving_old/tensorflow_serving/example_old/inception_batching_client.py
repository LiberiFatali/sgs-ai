...
tf.app.flags.DEFINE_string('server', 'localhost:9000',
                           'inception_batching_inference service host:port')
...
def do_inference(hostport, concurrency, listfile):
  ...
  stub = inception_batching_inference_pb2.beta_create_InceptionService_stub(channel)
  ...
  for imagefn in imagefns:
    ...
    #request.image_data = numpy.append(request.image_data, image_array.flatten().tolist())
    with cv:
