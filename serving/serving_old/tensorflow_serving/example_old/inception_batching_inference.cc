...
#include "tensorflow_serving/example/inception_batching_inference.grpc.pb.h"
#include "tensorflow_serving/example/inception_batching_inference.pb.h"
...
using tensorflow::serving::InceptionBatchingRequest;
...
class CallData {
 public:
  ...
  const InceptionBatchingRequest& request() { return request_; }
 ...
 private:
  ...
  InceptionBatchingRequest request_;
...
InceptionServiceImpl::InceptionServiceImpl(
  ...
  // HieuNM
  scheduler_options.thread_pool_name = "sgsnet_service_batch_gpu_threads";
//   scheduler_options.batch_timeout_micros = 1000 * 1000;  // 1 second
  scheduler_options.batch_timeout_micros = 10 * 1000 /* 10 milliseconds */;  
  scheduler_options.max_batch_size = 32;  
  scheduler_options.num_batch_threads = 4;
//   scheduler_options.num_batch_threads = 2;    
  //
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    LOG(ERROR) << "Usage: inception_batching_inference --port=9000 /path/to/exports";
    return -1;
  }
